...
do
  ...
  do
    ...
    mongo --eval "db.$COLLECTION_NAME.find({ loc : { \$near : $center, \$maxDistance : $radius}, frame:$counter });" mydb > /dev/null 2>&1
    ...
    START_TIME=$(date --iso-8601=ns | sed 's/,/./g')
    mongo --eval "db.$COLLECTION_NAME.find({ loc : { \$geoWithin : {\$center:[$center,$radius]} }, frame:$counter });" mydb > /dev/null 2>&1
    mongo --eval "db.system.profile.find( { ns : 'mydb.$COLLECTION_NAME', ts : {\$gt: new ISODate('$START_TIME') }, op:'query' },{millis:1} ).forEach(function(f){print(tojson(f, '', true));});" mydb \
      | sed 1,4d \
